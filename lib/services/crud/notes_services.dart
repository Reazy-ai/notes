...
class NotesService {
  ...
  Future<DatabaseNote> updateNote({
    ...
    if (updatesCount == 0) {
      ...
      _notes.removeWhere((note) => note.id == updatedNote.id); // Should remove oldNote.
  ...
  Future<Iterable<DatabaseNote>> getAllNotes() async {
    ...
    if (notes.isEmpty) {
      ...
      return notes.map(
        (eachNote) => DatabaseNote.fromRow(eachNote),
      );
    }
  }
  ...
  Future<void> deletUser({required String email}) async {
    ...
    final deletedCount = await db.delete(
      userTable,
      // why not just use email directly in the where clause, instead of putting it in the whereArgs?
      where: 'email = ?',
      whereArgs: [
        email.toLowerCase(),
      ],
